...
def add_a_snake():
    ...
    snake = service.add_snake(state.active_account, name, length, species, is_venomous)
    state.reload_account()
...
def view_your_snakes():
    ...
    snakes = service.get_snakes_for_user(state.active_account.id)
...
def book_a_cage():
    ...
    cages = service.get_available_cages(checkin, checkout, snake)
    ...
    service.book_cage(state.active_account, snake, cage, checkin, checkout)
...
def view_bookings():
    ...
    snakes = {s.id: s for s in service.get_snakes_for_user(state.active_account.id)}
    bookings = service.get_bookings_for_user(state.active_account.email)
